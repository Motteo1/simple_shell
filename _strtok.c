...
/**
 * t_size - returns number of delim ignoring continous delim
 * @str: user's command typed into shell
 * @delm: delimiter (e.g. " ");
 * Return: number of delims so that (num token = delim + 1)
 */
int t_size(char *str, char delm)
{
	...
	while (str[i] != '\0')
	{
		...
		{
			/* handle continous delims */
			num_delm++;
		}
		if ((str[i] == delm) && (str[i + 1] == '\0'))
		{
			/* handle continous delims after full command */
			num_delm--;
		}
		i++;
	}
...
char **_str_tok(char *str, char *delm)
{
	...
	while (str[se] != '\0') /* find string ending index */
		se++;
	while (si < se)
	{ /* malloc lenghts for each token ptr in array */
		if (str[si] != d_ch)
		{
		...
		/* handle repeated delimiters; increment ptr after ("ls __-l") */
